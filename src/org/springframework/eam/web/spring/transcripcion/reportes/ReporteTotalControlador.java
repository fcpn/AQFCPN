 ...
 * @jiwasana
 */
public class ReporteTotalControlador implements Controller {
    // METODOS PUBLICOS
	...
	String nombreArchivo = "ReporteTotal"+id_usuario+".pdf";
	PdfWriter.getInstance(document, new FileOutputStream("/opt/tomcat/webapps/cpReportes/reporteTotal/"+nombreArchivo));
	document.open();
	...
	document.add(imagen);
	document.add(new Paragraph("REPORTE TOTAL", titulo));
	...
	List reporteTotal = eam.getListaReporteTotal(reporte);
	int cantidad = reporteTotal.size();
	...
	for (int i = 0; i < cantidad; i++) {
	    Tarjeta reporteRecorrido = (Tarjeta)reporteTotal.get(i);
	...
	document.add(tabla);
	document.close();
	response.sendRedirect("/cpReportes/reporteTotal/"+nombreArchivo);
	modelo.put("archivo","/cpReportes/Prueba.pdf");
        return new ModelAndView("transcripcion/reportes/ReporteTotal",modelo);
    }
