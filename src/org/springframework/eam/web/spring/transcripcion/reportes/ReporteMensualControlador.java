 ...
 * @jiwasana
 */
public class ReporteMensualControlador implements Controller {
    // METODOS PUBLICOS
	...
	String nombreArchivo = "ReporteMensual"+id_usuario+".pdf";
	PdfWriter.getInstance(document, new FileOutputStream("/opt/tomcat/webapps/cpReportes/reporteMensual/"+nombreArchivo));
	document.open();
	...
	document.add(imagen);
	document.add(new Paragraph("REPORTE MENSUAL", titulo));
	...
	List reporteMensual = eam.getListaReporteMensual(reporte);
	int cantidad = reporteMensual.size();
	...
	for (int i = 0; i < cantidad; i++) {
	    Tarjeta reporteRecorrido = (Tarjeta)reporteMensual.get(i);
	...
	document.add(tabla);
	document.close();
	response.sendRedirect("/cpReportes/reporteMensual/"+nombreArchivo);
	modelo.put("archivo","/cpReportes/Prueba.pdf");
        return new ModelAndView("transcripcion/reportes/ReporteMensual",modelo);
    }
